...
let readManifest, writeManifest, credential, isAuthRequired;
if (config) {
  if (
...
if (config) {
  if (
    typeof config.credential === "object" ||
    ...
    throw new Error(
      `Invalid config file -- username and password should either be object or missing completely`
    );
  }
}
