...
const express = require("express"),
  ...
  { isTrustedHost } = require("./trusted-urls");
...
app.patch("/import-map.json", (req, res) => {
  ...
  if (req.body.imports) {
    ...
    const unsafeUrls = importUrlsToValidate
      .map((host) => {
        return !isTrustedHost(host, env) ? host : null;
      })
      .filter(Boolean);
  ...
  if (req.body.scopes) {
    ...
    const unsafeUrls = scopeUrlsToValidate
      .map((host) => {
        return !isTrustedHost(host, env) ? host : null;
      })
      .filter(Boolean);
